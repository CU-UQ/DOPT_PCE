...
% matlab -nodesktop -nodisplay <test_problem_duffing.m &> file.out & 
...
order = 12; %try order = 9 or 12
load_name = ['duffing_recon_data_p',num2str(order)];
load(load_name)
load_name = ['duffing_validation_data_p',num2str(order)];
load(load_name)
sample_sizes = round(linspace(20,P-10,10))
...
for n = 1:Num_samps %Iterate to next sample size
    ...
    clear Psi 
    save_name = ['coh_opt_duffing',num2str(dim),'d_',num2str(order),'p_',num2str(R),'R'];
    %save(save_name)
...
% 
% inputs = zeros(size(sample.rv));
% inputs(:,1) = 0.05*(1.0+0.05.*sample.rv(:,1)); % Xi
% inputs(:,2) = 2*pi*(1.0+0.2.*sample.rv(:,2));  % Omega 
% inputs(:,3) = -0.5*(1.0+0.5.*sample.rv(:,3));  % eps
%     
% %global eps omega xi
% u = zeros(M,4);
% W = zeros(M,1);
...
%     Inputs = fliplr(inputs(m,:))
%     u(m,:) = le_duffing_call(Inputs);
%     B = norm(psi(m,:),2);
%     W(m) = 1/B;
% end
% sample.lhs = psi;
...
% 
% time = toc
% save_name = ['duffing_recon_data_p',num2str(order)]
% % save(save_name)
...
% inputs = zeros(size(sample.rv));
% 
% inputs(:,1) = 0.05*(1.0+0.05.*sample.rv(:,1));
% inputs(:,2) = 2*pi*(1.0+0.2.*sample.rv(:,2));
% inputs(:,3) = -0.5*(1.0+0.5.*sample.rv(:,3));
% 
% u = zeros(M,4);
% psi = zeros(M,P);
% 
...
% 
% time = toc
% save_name = ['duffing_validation_data_p',num2str(order)]
